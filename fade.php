...
I also added the (albeit basic) dice rolling animation effect. If inline SVGs are not supported, 
the user is redirected to a different (PHP-generated) version of the page that uses <object> tags 
to display SVG images in the page. Because of the way the object tag works, if SVG images are not 
supported in the user's web browser, rasterized and highly compressed PNG images will be displayed.
...
?>	roll();
	...
	function animate() {
		...
		animInterval = window.setInterval('roll()',3);
	}

	function roll() {
		dice[0] = Math.floor(Math.random() * 6) + 1;
		dice[1] = Math.floor(Math.random() * 6) + 1;
		$("#die1-" + dice[0]).fadeTo("fast", 1.00);
		$("#die2-" + dice[1]).fadeTo("fast", 1.00);
		for (c = 1;c < 7; c++)	{
			for (d = 0; d < 2; d++) {
				if (c != dice[d]) {
					$("#die" + (d+1) + "-" + c).fadeTo(50, 0.00);
					//document.getElementById(("die" + (d+1) + "-" + c)).style.display='none';
				} else { 
					
					//document.getElementById(("die" + (d+1) + "-" + c)).style.display='block'; 
				}
			}
		}
		<?php #hidden feature! rolls the dice endlessly.
		if (!isset($_GET["haxx"])) {
			echo "if ((rolls = rolls + 1) > 10) {\n			clearInterval(animInterval);\n		}"; 
		} ?>
	}
</script>
</head>
